    ...
    with gr.Tabs():
        ...
        with gr.Tab("Generate Lines") as gen_lines_tab:
            with gr.Row():
                ...
                with gr.Column():
                    gen_lines = gr.TextArea(label="Lines", interactive=False, visible = False)
                    gen_info = gr.TextArea(label="Info", interactive=False, visible = False)
                    
                    temperature = gr.Slider(label="Temperature (Higher = more creative, lower = more predictable response)", value=0.7, minimum=0.0, maximum=1.0, step=0.01, interactive=True, visible=False)
                    top_p = gr.Slider(label="Top P (Higher = larger set of words to choose from, lower = choose from the most likely words)", value=0.5, minimum=0.0, maximum=1.0, step=0.01, interactive=True, visible=False)
